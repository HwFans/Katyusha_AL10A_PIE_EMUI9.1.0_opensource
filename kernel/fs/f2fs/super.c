...
unsigned int write_opt_v2 = 0;
static unsigned int zero = 0;
static unsigned int one = 1;
char *model_name[] = {"MT6762V/CR", "MT6762V/CS", "MT6762V/WR",
			"MT6762V/WS", "MT6762R"};
...
static unsigned int enable_life_model(void)
{
	int i;
	...
	if (machine_name == NULL) {
		printk_ratelimited("get machine_name fail\n");
		return write_opt_v2;
	}
	for (i = 0; i < sizeof(model_name) / sizeof(char *); i++) {
		if (!memcmp(machine_name, model_name[i],
			strlen(model_name[i]))) {
			write_opt_v2 = 1;
			break;
		}
...
static int __init pw_init(void){
	...
	if (!register_sysctl_table(f2fssys_table))
		return -ENOMEM;
	ret = enable_life_model();
	printk_ratelimited("enable_life_model = %d\n", ret);
	return 0;
}
#endif
